...
- M0（地基）+ M1（生产级 MySQL 全部功能）**已实现，并已完成首轮逐层代码评审**（证据见 `docs/design/review.md`），但**尚未进入「定方案 → 修复」阶段**。
- `docs/design/defects.md` 现列 **69 项缺陷**：首批 20 项（🔴4/🟡12/🟢4）+ 评审新增 49 项（🔴7/🟡28/🟢14，#21–#69）。
- 缺陷修复按铁律分规模执行：小缺陷直接修；中/大缺陷先定方案、评审通过后再修。
- 修复顺序建议：🔴 → 🟡 → 🟢；架构级缺陷需一次性定方案避免返工——首批 `#9`（SSH 指纹）、`#5`（秒断取消）、`#4`（前端方言感知）、`#1`（类型映射）；新增 `#21`（全局锁串行化）、`#22`（SSH 明文落盘）、`#23`（sticky cancel）、`#24`（SSH 隧道泄漏）。
