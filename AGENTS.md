...
- `docs/design/defects.md` 现列 **81 项缺陷**：档位 C 的 10 个方案组（及其覆盖的 `#3/#19/#21/#23/#34/#36/#42/#55/#59/#61/#63` 等次级缺陷）已勾掉；档位 B（需评审）与档位 A（直接修）的其余缺陷仍待处理。
...
- **最近修复**：
  ...
  - `#77`（函数/存储过程 DDL 显示 sql_mode 而非 DDL，P1 小型）已修复 — 修复 `show_create_object` 返回错误列，根据对象类型选择正确的列索引。
  - `#78`（缺少执行存储过程的快捷方式，P2 小型）已修复 — 添加 `execute_procedure` 命令，存储过程节点右键菜单添加"执行"选项。
  - `#79`（DDL 模板缺少 DETERMINISTIC 特性，P1 小型）已修复 — 函数 DDL 模板添加 `DETERMINISTIC` 关键字满足 MySQL 二进制日志要求。
  - `#80`（视图 DDL 显示未格式化的单行 SQL，P2 小型）已修复 — 添加 `format_sql` 函数在主要关键字前添加换行，改善 DDL 可读性。
  - `#81`（查询数据/查看DDL功能执行SQL而非插入编辑器，P1 小型）已修复 — 统一所有命令只生成 SQL 语句并插入编辑器，用户可查看/编辑后手动执行。
