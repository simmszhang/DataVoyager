...
- `docs/design/defects.md` 现列 **75 项缺陷**：档位 C 的 10 个方案组（及其覆盖的 `#3/#19/#21/#23/#34/#36/#42/#55/#59/#61/#63` 等次级缺陷）已勾掉；档位 B（需评审）与档位 A（直接修）的其余缺陷仍待处理。
...
- **最近修复**：
  ...
  - `#75`（Schema 树缺少创建对象的右键菜单，P2 中型）已修复 — 在表/视图/函数/存储过程分组节点添加右键菜单，点击插入对应 DDL 模板到编辑器。
