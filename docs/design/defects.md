...
| # | 缺陷 | P | 规模 |
|---|---|---|---|
| #9 | SSH 隧道接受任意主机密钥（MITM） | P0 | 大 |
| #22 | SSH 密码/私钥明文落盘 + 经 IPC 返回前端 | P0 | 大 |
| #48 | `csp:null` + 破坏性命令无 ACL 门控 | P0 | 大 |
| #1 | Value 类型映射启发式（含 #20/#32/#33/#60） | P1 | 大 |
| #4 | 前端硬编码 MySQL 反引号（方言感知） | P1 | 大 |
| #5 | 取消 drain 式（含 #21/#23/#34） | P1 | 大 |
| #24 | SSH 隧道断开后资源泄漏 | P1 | 大 |
| #28 | 多结果集无边界事件（协议层） | P1 | 大 |
| #45 | BIGINT/主键 >2^53 跨 IPC 丢精度 | P1 | 大 |
| #18 | 无 i18n（框架先行） | P3 | 大 |
...
### #23 取消令牌每连接一个且永不 reset（sticky cancel）
...
### #42 流式通道无终止/错误事件 + `channel.send` 失败被吞
...
### #45 数值精度：`I64/U64`/id/last_insert_id 以 JSON number 跨 IPC，>2^53 丢精度
...
### #48 安全面：`csp: null` + 自定义 command 无 ACL 门控
...
### #59 `split_statements` 不支持 dollar-quoting 等方言差异
- **现状**：只处理 `'`/`"`/反引号 + `--`/`#`/`/* */`。
...
### #63 连接配置无更新命令、`color` 死字段、驱动参数不持久化
